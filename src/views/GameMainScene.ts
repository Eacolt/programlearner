...
export default class GameMainScene extends PIXI.Container {
    ...
    private actionUpper_canTouch:boolean = false;//上部分命令是否可以点击;
    ...
    //代码区域减去一行
    private codeMinus(_index: number = -1) {
        ...
        if (this._digitalLine > 12) {
            ...
            if (this.addedAction_arr.length > 0) {
                if (_index < 0) {
                  ...
                  let currentSprit: PIXI.Sprite = this.addedAction_arr[_index] as PIXI.Sprite;
                  currentSprit.parent.removeChild(currentSprit);
               
                  for(let i=_index+1;i<this.addedAction_arr.length;i++){
                      this.addedAction_arr[i].y -= this.distVerLine;

                  }
                  this.addedAction_arr.splice(_index,1);
        ...
        this.renameAllActionBar();
    }
    //对所有命令模块重命名;
    private renameAllActionBar() {
        for (let i = 0; i < this.addedAction_arr.length; i++) {
            this.addedAction_arr[i].name = i + '_bar'
        }
    }
    //代码区域增加一行
    private codeAdded() {
        ...
        if (this._digitalLine > 13) {
            //命令块拖曳程序
            let actionBar = new PIXI.Sprite(Labs.getTexture(this.actionDown_arr[this.choosedBarIndex]));

            actionBar.interactive = true;

            this.scrollUI.addContent(actionBar);
            this.addedAction_arr.push(actionBar);
            // actionBar.name = 'bar'+(this.addedAction_arr.length-1);
            if (this.addedAction_arr.length > 1) {
                ...
                actionBar.x = this.startcoding.x;
                actionBar.y = this.firstActionPos;
                this.startcoding.y += this.distVerLine;
            }
            setTimeout(()=>{
                actionBar.on('pointerdown', this.actionBar_downHandler, this);


                actionBar.on('pointerup', this.actionBar_upHandler, this);
                actionBar.on('pointerupoutside', this.actionBar_upHandler, this);
    ...
    private actionBar_downHandler(event: any) {
        ...
        if(!this.actionUpper_canTouch)return;
        this.scrollUI.lock = true;
        console.log('怎么回事啦啦啦啦')
        this.pickedUpActionBar.interactive = true;
        this.pickedUpActionBar.renderable = true;
        this.pickedUpActionBar.x = event.data.global.x;
        this.pickedUpActionBar.y = event.data.global.y;
        this.pickedUpActionBar.on('pointermove', this.actionBar_moveHandler, this);
        console.log('选中啦。。。', event.currentTarget.name)
    };
    ...
    private actionBar_upHandler(event: any) {
        if(!this.actionUpper_canTouch)return;
        this.scrollUI.lock = false;//解锁；
        this.pickedUpActionBar.off('pointermove');
        this.pickedUpActionBar.renderable = false;
        console.log('删除啦',parseInt(event.currentTarget.name))

        this.codeMinus(parseInt(event.currentTarget.name));
    ...
    private bottomBtn_upHandler(index: number, event: any) {
        ...
        setTimeout(()=>{
            this.actionUpper_canTouch = true;
        },10)


    }
    private bottomBtn_downHandler(data: any, event: any) {
        this.actionUpper_canTouch = false;
